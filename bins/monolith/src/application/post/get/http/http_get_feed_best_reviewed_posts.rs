...
pub async fn handle_get_best_reviewed_posts<T: GetPostsContract>(
    req: HttpRequest,
    service: web::Data<T>,
    ...
    let response = match user {
        Some(user) => {
                service
                    .get_feed_best_reviewed_posts_paginated(Some(user.id), attributes)
                    .await?
        },
        None => {
            service
                .get_feed_best_reviewed_posts_paginated(None, attributes)
                .await?
        }
    };
